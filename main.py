...
def prepare_posedirs(posedirs: torch.Tensor, expected_pose_dim: int):
    ...
    if posedirs.shape[1] == expected_pose_dim:
        return posedirs.T
    raise RuntimeError(f"posedirs 维度不匹配")


# ========================================================
# 核心补全：实现完全无错、自适应维度的手写 LBS 逻辑
# ========================================================
def compute_manual_lbs(model, betas, global_orient, body_pose):
    ...
    # (d) 线性混合蒙皮 (Linear Blend Skinning)
    J_transformed, A = batch_rigid_transform(rot_mats, J, model.parents, dtype=dtype)

    num_joints = J.shape[1]
    # 使用官方类属性名：model.lbs_weights 确保完美向下兼容
    W = model.lbs_weights.unsqueeze(0).expand(1, -1, -1)  # [1, V, K]
...
def main(args):
    ...
    # 路径解析：自动支持传入绝对或相对路径
    if os.path.exists(args.model_dir) and os.path.isfile(os.path.join(args.model_dir, "smpl/SMPL_NEUTRAL.pkl")):
        model_path_target = os.path.join(args.model_dir, "smpl/SMPL_NEUTRAL.pkl")
    elif os.path.exists(args.model_dir) and os.path.isfile(os.path.join(args.model_dir, "SMPL_NEUTRAL.pkl")):
        model_path_target = os.path.join(args.model_dir, "SMPL_NEUTRAL.pkl")
    else:
        model_path_target = resolve_script_path(os.path.join(args.model_dir, "smpl/SMPL_NEUTRAL.pkl"))
    ...
    # 自动合并拼接成 2x2 的全阶段对比网格大图 (comparison_grid.png)
    grid_dict = {
    ...
    save_all_joint_weights_figure(
        ...
        to_numpy(data["v_template"][0]), faces,
        to_numpy(data["J_template"][0]), to_numpy(model.lbs_weights),
    )
    ...
    print("运行完成。")
    print(f"顶点数: {num_vertices} | 面片数: {num_faces} | 关节数: {num_joints}")
    print(f"平均绝对误差 (MAE): {mean_err:.10f}")
    print(f"最大绝对误差 (Max AE): {max_err:.10f}")
    print(f"所有渲染成果图及数据摘要已成功输出至: {out_dir}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # 🌟 将 default 从 "./models" 改为 "."
    parser.add_argument("--model-dir", type=str, default=".", help="模型目录")
